            ...
            <table className="w-full text-sm text-left" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr className="border-b" style={{ borderBottom: '1px solid #eee' }}>
                  <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: '600', color: '#666' }}>Order ID</th>
                  <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: '600', color: '#666' }}>Customer</th>
                  <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: '600', color: '#666' }}>Status</th>
                  <th style={{ padding: '10px 12px', textAlign: 'right', fontWeight: '600', color: '#666' }}>Amount</th>
                </tr>
              </thead>
              <tbody>
                {stats?.recentOrders.map((order: any) => (
                  <tr key={order.id} className="border-b hover:bg-gray-50" style={{ borderBottom: '1px solid #f7f6f2' }}>
                    ...
                    <td style={{ padding: '12px', color: '#444' }}>{order.shippingName}</td>
                    <td style={{ padding: '12px' }}>
                      <span
                        className={`status-tag ${order.orderStatus.toLowerCase()}`}
                        style={{
                          fontSize: '11px',
                          padding: '3px 8px',
                          borderRadius: '4px',
                    ...
                    <td style={{ padding: '12px', textAlign: 'right', fontWeight: '600' }}>₹{order.total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card p-6 border rounded" style={{ background: '#fff', padding: '24px', borderRadius: '8px', border: '1px solid #e9e5df', boxShadow: '0 1px 3px rgba(0,0,0,0.03)' }}>
          ...
          <div className="table-responsive" style={{ overflowX: 'auto' }}>
            <table className="w-full text-sm text-left" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr className="border-b" style={{ borderBottom: '1px solid #eee' }}>
                  <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: '600', color: '#666' }}>Product Name</th>
                  <th style={{ padding: '10px 12px', textAlign: 'right', fontWeight: '600', color: '#666' }}>Units Sold</th>
                </tr>
              </thead>
              <tbody>
                {stats?.bestSellers.map((item: any, i: number) => (
                  <tr key={i} className="border-b" style={{ borderBottom: '1px solid #f7f6f2' }}>
                    ...
                    <td style={{ padding: '12px', textAlign: 'right', fontWeight: '600', color: '#444' }}>{item.quantitySold} units</td>
                  </tr>
                ))}
              </tbody>
            </table>
